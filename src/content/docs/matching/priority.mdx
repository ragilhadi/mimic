...
A request whose literal path matches nothing may still be served via [trailing-slash tolerance](/matching/trailing-slash/) — the match explanation on such a response notes `trailing slash normalized`, with no score penalty for taking that fallback.
...
Equal scores are broken **deterministically** — the winner never depends on load order, so it stays the same across restarts and hot reloads:

1. **Most literal path segments first** — `/users/:id` (one parameter) beats `/{resource}/:id` (two parameters) for the same score.
2. **Lowest `METHOD:path` key, lexicographically.**
3. **Earliest position** among mocks sharing that key — effectively, alphabetical-by-file-path load order (see [Mock Files](/guides/mock-files/#how-mimic-discovers-mocks)).

This makes ties predictable rather than an error to design around, but a mock that depends on winning a tie is still harder to reason about than one that doesn't — adding a matcher to push the score apart remains the clearest way to make a mock's precedence obvious at a glance.
